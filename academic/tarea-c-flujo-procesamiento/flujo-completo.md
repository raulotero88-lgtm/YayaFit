...
El flujo de 3 pasos mantuvo íntegras todas las ideas del input original: el trato cercano con Yaya Fitters, el modelo de clases diferidas con límites en plan estándar y acceso completo en premium, y el objetivo de 30.000 usuarias activas se conservaron sin pérdida semántica a través de las transformaciones. El filtro de fechas funcionó efectivamente: "este año" fue mantenida como referencia relativa y marcada como [PENDIENTE — confirmar fecha], sin conversiones a fechas concretas. El test de coherencia inter-capítulos no detectó desviaciones entre Cap. 1 y Cap. 3 en términos de información; sin embargo, se halló un fallo de coherencia **intra-capítular dentro del Cap. 1 mismo**: el modelo utilizó inconsistentemente "modelo estándar" y "plan premium" en el mismo párrafo, oscilando entre "modelo" y "plan" como sustantivo sin causa externa, demostrando un drift terminológico espontáneo. Este error de coherencia interna es el más fácil de reproducir con este input — aparece de forma consistente en la generación libre sin necesidad de cadenas de prompts complejas, simplemente por la ausencia de un glosario terminológico vinculante en el system prompt. El hallazgo demuestra que en sistemas de generación multi-sección es imprescindible anclar los nombres de entidades y términos clave como variables fijas o glosarios iniciales para evitar derivas terminológicas que socavan la profesionalidad del documento final.
