...
- **Identidad (REST /settings):** título "YayaFit", tagline "Tu segunda juventud empieza aquí", zona horaria Europe/Madrid, fecha `j \d\e F \d\e Y`, hora `H:i`, email admin → **contacto@yayafit.es** (decidido el mismo día tras descartar el personal de Raúl y un Gmail nuevo que no podía verificar; ⚠️ **crear el buzón en hPanel de Hostinger → Correo electrónico → yayafit.es**, ahí llegan el formulario de contacto y los avisos de WordPress; se puede añadir reenvío al Gmail personal), pingbacks cerrados.
